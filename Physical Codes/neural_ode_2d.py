...
true_y0 = torch.tensor([[2., 0.]]).to(device)# initialize y0
t = torch.linspace(0., 25., args.data_size).to(device)# define points to evaluate
true_A = torch.tensor([[-0.1, 2.0], [-2.0, -0.1]]).to(device)### defining a weight matrix???
...
class Lambda(nn.Module):
    ...
    def forward(self, t, y):
        ...
        return torch.Tensor([np.cos(t/20),np.sin(t/20)])
...
def visualize(true_y, pred_y, odefunc, itr):# define the visualization process
    ...
    if True:
        ...
        ax_traj.plot(t.cpu().numpy(), true_y.cpu().numpy()[:, 0, 0], t.cpu().numpy(), true_y.cpu().numpy()[:, 0, 1], 'g-') # green is gound truth
        ax_traj.plot(t.cpu().numpy(), pred_y.cpu().numpy()[:, 0, 0], '--', t.cpu().numpy(), pred_y.cpu().numpy()[:, 0, 1], 'b--') # blue is prediction
        ...
        ax_phase.plot(true_y.cpu().numpy()[:, 0, 0], true_y.cpu().numpy()[:, 0, 1], 'g-') # green is ground truth
        ax_phase.plot(pred_y.cpu().numpy()[:, 0, 0], pred_y.cpu().numpy()[:, 0, 1], 'b--') # blue is prediction
