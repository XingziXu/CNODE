...
class Grad_net(nn.Module): # the Grad_net defines the networks for the path and for the gradients
    def __init__(self, width_path: int, width_grad: int, width_conv2: int, width_aug: int):
        super().__init__()
        self.nfe=0 # initialize the number of function evaluations
        
        self.conv2 = nn.Conv2d(width_aug+3,width_conv2,1)
        ...
        self.grad_g = nn.Sequential( # define the network for the gradient on x direction
            ...
            nn.GroupNorm(3+width_aug+3,3+width_aug+3),
            nn.Conv2d(3+width_aug+3,width_grad,1,1,0),
            #nn.Softplus(),
            nn.ReLU(),
            ...
            nn.Conv2d(width_grad,3+width_aug,1,1,0)
        )
...
def main():
    ...
    optimizer_grad = optim.AdamW(list(grad_net.grad_g.parameters())+list(grad_net.grad_h.parameters())+list(grad_net.grad_i.parameters())+list(grad_net.conv2.parameters()), lr=args.lr_grad, weight_decay=args.weight_decay) # define optimizer on the gradients
