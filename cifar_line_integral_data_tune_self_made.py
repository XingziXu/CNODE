...
def main():
    ...
    validation_loader = torch.utils.data.DataLoader(dataset2, **test_kwargs)
    ...
    ############################################ Self made parameter search starts here ##################################################
    r1 = 1e-4
    r2 = 1e-2
    lr_mat = ((r1 - r2) * torch.rand(20,1) + r2)

    result = torch.zeros(20,2)
    

    for idx_test in range(0,19):

        print("the learning rate in search number ", idx_test, " is", lr_mat[idx_test].item())

        optimizer = optim.AdamW(list(encoder.parameters())+list(path_net.parameters())+list(grad_x_net.parameters())+list(grad_y_net.parameters()), lr=lr_mat[idx_test].item())
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            result[idx_test,0] = lr_mat[idx_test]
            result[idx_test,1] = validation(args, encoder, path_net, grad_x_net, grad_y_net, device, validation_loader)
            print("we have finished the search number", idx_test, "the accuracy is ", result[idx_test,1], "the lr is ", result[idx_test,0])
            scheduler.step()
        plt.plot(result[:,0],result[:,1])
        #plt.show()
        plt.savefig('learning_rate.png', dpi=None, facecolor='w', edgecolor='w',
    ...
    print(result)
    torch.save(result, 'learning_rate.pt')
