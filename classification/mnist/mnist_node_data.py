...
class Depth_net(nn.Module): # the Grad_net defines the networks for the path and for the gradients
    def __init__(self, width_depth: int):
        super().__init__()
        self.depth = nn.Sequential( # define the network for the gradient on x direction
        nn.Conv2d(1,width_depth, 3, padding=1, bias=False),
        nn.Hardtanh(),
        nn.Conv2d(width_depth,width_depth, 3, padding=1, bias=False),
        nn.Hardtanh(),
        nn.Conv2d(width_depth,3,1),
        nn.Flatten(),
        nn.Linear(2352,1),
        nn.Sigmoid()
        )
...
class Grad_net(nn.Module): # the Grad_net defines the networks for the path and for the gradients
    ...
    def forward(self, t, x):
        ...
        dp = self.grad_g(x.unsqueeze(0)) # calculate the change in p
        return dp
...
def update(args, grad_net, classifier_net, depth_net, optimizer, data, target, device):
    ...
    for i in range(0,p.size(0)):
        t = torch.Tensor([0.,times[i,0]]).to(device) # we look to integrate from t=0 to t=1
        t.requires_grad=True # record the computation graph
        if args.adaptive_solver: # check if we are using the adaptive solver
            p[i,:,:] = torch.squeeze(odeint_adjoint(grad_net, p[i,:,:], t,method="dopri5",rtol=args.tol,atol=args.tol)[1]) # solve the neural line integral with an adaptive ode solver
            ...
            output = torch.cat((output,odeint(grad_net, p[i,:,:], t, method="euler")[1].unsqueeze(0)),0) # solve the neural line integral with the euler's solver
            grad_net.nfe=0 # reset the number of function of evaluations
    output = classifier_net(grad_net.conv2(output)) # classify the transformed images
...
def evaluate(args, grad_net, classifier_net, depth_net, data, device):
    p = data # assign data, initialization
    p.requires_grad=True # record the computation graph
    times = depth_net(p)
    output = torch.empty(0).to(device)
...
def train(args, grad_net, classifier_net, depth_net, device, train_loader, optimizer_grad, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader): # for each batch
        ...
        p_i = data
        loss_grad = update(args, grad_net, classifier_net, depth_net, optimizer_grad, data, target, device) # update gradient networks' weights
...
def validation(args, grad_net, classifier_net, depth_net, device, validation_loader):
    ...
    for data, target in validation_loader: # for each data batch
        ...
        global p_i # claim the initial image batch as a global variable
        p_i = data
        output = evaluate(args, grad_net, classifier_net, depth_net, data, device)
...
def main():
    ...
    parser.add_argument('--width-depth', type=int, default=8, metavar='LR',
                        help='width of the gradient network')
