...
x = np.linspace(0,1,num=50) # initialize x values
y = np.linspace(0,1,num=50) # initialize y values
#z = np.linspace(0,1,num=50) # initialize z values
t = np.linspace(0,1,num=50) # initialize t values
data = [[1],[1]]
...
for i in range(1,len(t)): # for each time step
  for j in range(1,len(x)): # for each x step
    for k in range(1,len(y)): # for each y step
        ...
        data = np.append(data, [[U_0+phi1*x[i]+phi2*y[j]]],axis=0)
