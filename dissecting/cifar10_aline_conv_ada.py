...
class Grad_net(nn.Module): # the Grad_net defines the networks for the path and for the gradients
    def __init__(self, width_path: int, width_grad: int, width_conv: int, width_aug: int):
        ...
        self.conv = nn.Conv2d(width_aug+3, width_conv, 1)
        ...
        self.grad_g = nn.Sequential( # define the network for the gradient on x direction
            nn.GroupNorm(width_aug+3+3,width_aug+3+3),
            nn.Conv2d(width_aug+3+3,width_grad, 3, padding=1, bias=False),
            nn.ReLU(),
            nn.Conv2d(width_grad,width_grad, 3, padding=1, bias=False),
            nn.ReLU(),
            nn.GroupNorm(width_grad,width_grad),
            nn.Conv2d(width_grad,width_aug+3,1)
        )
...
def update(args, grad_net, classifier_net, optimizer, data, target, device):
    ...
    if args.adaptive_solver: # check if we are using the adaptive solver
        ...
        grad_net.nfe=0 # reset the number of function of evaluations
    output = classifier_net(grad_net.conv(p)) # classify the transformed images
...
def main():
    ...
    parser.add_argument('--width-aug', type=int, default=39, metavar='LR',
                        help='number of augmentation dimension')
    parser.add_argument('--width-conv', type=int, default=6, metavar='LR',
                        help='width of convolution')
    parser.add_argument('--width-pool', type=int, default=4, metavar='LR',
                        help='width of average pooling')
    ...
    grad_net = Grad_net(width_path=args.width_path, width_grad=args.width_grad, width_aug=args.width_aug, width_conv=args.width_conv).to(device) # define grad_net and assign to device
    ...
    optimizer_grad = optim.AdamW(list(grad_net.grad_g.parameters())+list(grad_net.grad_h.parameters())+list(grad_net.grad_i.parameters())+list(grad_net.conv.parameters()), lr=args.lr_grad, weight_decay=5e-4) # define optimizer on the gradients
