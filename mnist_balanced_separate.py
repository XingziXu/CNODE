...
class Grad_net(nn.Module): # the Grad_net defines the networks for the path and for the gradients
    def __init__(self, width_path: int, width_grad: int, width_conv2: int):
        ...
        self.top_g = nn.Sequential( # define the network for the gradient on x direction
            nn.InstanceNorm2d(1),
        ...
        self.top_h = nn.Sequential( # define the network for the gradient on y direction
            nn.InstanceNorm2d(1),
            nn.Conv2d(1,width_grad,1,1,0),
            nn.ReLU(),
            nn.Conv2d(width_grad,width_grad,3,1,1),
    ...
    def forward(self, t, x):
        ...
        top_g = self.top_g(x)
        top_h = self.top_h(x)
        grad_g = torch.autograd.grad(top_g, x, torch.ones_like(top_g))[0][:,1:,:,:]
        grad_h = torch.autograd.grad(top_h, x, torch.ones_like(top_h))[0][:,1:,:,:]

        dp = torch.mul(grad_g,dg_dt) + torch.mul(grad_h,dh_dt)# + torch.mul(self.grad_i(x),di_dt) + torch.mul(self.grad_k(x),dk_dt) + torch.mul(self.grad_l(x),dl_dt) # calculate the change in p
        #print(t.item())
        return dp
