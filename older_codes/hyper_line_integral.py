...
def train(args, encoder, path_net, grad_x_net, grad_y_net, grad_z_net, device, train_loader, optimizer, epoch):
...
#    grad_y_net = grad_y_net.to(device)
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        for iter in range(1,int(args.num_eval)+1): # for each random value, integrate from 0 to 1
            ...
            g_h_k_current = path_net(t_current)
            torch.sum(g_h_k_current).backward(retain_graph=True)
            dg_dt_current = torch.autograd.grad(g_h_k_current[0], t_current, retain_graph = True)[0] # calculate the current dg/dt
            dh_dt_current = torch.autograd.grad(g_h_k_current[1], t_current, retain_graph = True)[0]
            dk_dt_current = torch.autograd.grad(g_h_k_current[2], t_current, retain_graph = True)[0]
            in_grad = torch.cat((p_current.view(p_current.size()[0], 10), g_h_k_current.repeat([p_current.size()[0],1]).view(p_current.size()[0],3)), dim=1).to(device)
            #p_current = p_current + dt*(torch.dot(torch.cat((grad_x_net(in_grad), grad_y_net(in_grad)),dim=1),dg_dh_dt_current))
            p_current = p_current + dt*(grad_x_net(in_grad)*dg_dt_current + grad_y_net(in_grad)*dh_dt_current + grad_z_net(in_grad)*dk_dt_current)
...
def test(args, encoder, path_net, grad_x_net, grad_y_net, grad_z_net, device, test_loader):
#    encoder = encoder.to(device)
#    path_net = path_net.to(device)
#    grad_x_net = grad_x_net.to(device)
#    grad_y_net = grad_y_net.to(device)
...
def validation(args, encoder, path_net, grad_x_net, grad_y_net, grad_z_net, device, validation_loader):
#    encoder = encoder.to(device)
#    path_net = path_net.to(device)
#    grad_x_net = grad_x_net.to(device)
#    grad_y_net = grad_y_net.to(device)
...
#    dataset4, dataset2 = torch.utils.data.random_split(dataset2, [9990,10])
    ...
    grad_z_net = Grad_net(input_size_grad, width_grad, output_size_grad)
    optimizer = optim.SGD(list(encoder.parameters())+list(path_net.parameters())+list(grad_x_net.parameters())+list(grad_y_net.parameters())+list(grad_z_net.parameters()), lr=args.lr)
    ...
    for epoch in range(1, args.epochs + 1):
        train(args, encoder, path_net, grad_x_net, grad_y_net, grad_z_net, device, train_loader, optimizer, epoch)
        validation(args, encoder, path_net, grad_x_net, grad_y_net, grad_z_net,device, test_loader)
        scheduler.step()
    test(args, encoder, path_net, grad_x_net, grad_y_net, grad_z_net,device, validation_loader)
