...
def visualize(true_t, true_h, pred_t, pred_h, loss, num_eva):# define the visualization process
...
#        makedirs('png')# if there is not a png image already, create one
        ...
        ax_traj.plot(true_t,true_h, 'g-') # green is gound truth
        ax_traj.plot(pred_t.view([int(num_eva),1]).detach().numpy(), pred_h.view([int(num_eva),1]).detach().numpy(), 'b--') # blue is prediction
...
if __name__ == '__main__':
    ...
    params = list(h_learn.parameters()) # define parameters for the combined model
    ...
    for itr in range(1, iteration_num + 1):
        ...
        for iter in range(int(num_eval)): # for each random value, integrate from 0 to 1
            ...
            dgdt_current = torch.Tensor([[1.]]) # calculate the current dg/dt
            ...
            g_current = torch.Tensor([[[t_current]]])
        ...
        # calculate the trained integration path for visualization
            ...
            visualize(t_true, h_true, t_prediction, h_prediction, loss_vis, num_eval)
