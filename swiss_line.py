...
class ShiftedSines(Dataset):
    ...
    def __init__(self, dim, shift, num_points_upper, num_points_lower,
        ...
        for i in range(self.num_points_upper + self.num_points_lower):
            ...
            phi =  -2 * pi * torch.rand(1)  # Random point between -1 and 1
...
def main():
    ...
    with open('train_loss_swiss_line3.npy', 'wb') as f:
        np.save(f, np.asarray(loss_train))
    with open('test_loss_swiss_line3.npy', 'wb') as f:
        np.save(f, np.asarray(loss_test))
    with open('accuracy_swiss_line3.npy', 'wb') as f:
